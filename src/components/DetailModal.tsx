...
export default function DetailModal() {
  ...
  const handleOpenOcrWorkspace = async () => {
    const dataUrl = currentDisplayImageSrc || currentOutputPreviewSrc
    if (!dataUrl) {
      showToast('当前没有可处理的图片', 'error')
      return
    }
    const imageId = currentOutputImageId || currentOriginalOutputImageId || task.outputImages[0] || ''
    if (!imageId) {
      showToast('当前图片尚未准备好', 'error')
      return
    }
    useStore.getState().setOcrImage({ id: imageId, dataUrl })
    useStore.getState().addOcrImages([{ id: imageId, dataUrl }])
    useStore.getState().setAppMode('gallery')
    useStore.getState().setGalleryMode('ocr')
      ...
      >
        ...
        <div className="md:w-1/2 w-full p-5 overflow-y-auto overscroll-contain flex flex-col">
          ...
          <div className="grid grid-cols-4 sm:flex gap-2 pt-4 border-t border-gray-100 dark:border-white/[0.08]">
            ...
            <button
              onClick={() => void handleOpenOcrWorkspace()}
